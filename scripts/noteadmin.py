...
NOTESERVER_URL = 'http://localhost:3002'
...
def post_bulk(remote_dir, local_folder):
    ...
    for entry in to_post:
        ...
        resp = requests.post(NOTESERVER_URL + '/' + remote_dir + '/' + entry[0], md_content, headers=auth_headers)
...
while True:
    ...
    try:
        if (args[0] == 'q' or args[0] == 'exit'):
            ...
            resp = requests.get(NOTESERVER_URL + '/all', headers=auth_headers)
            print(resp.text)
        elif (args[0] == 'post'):
            remote_note = args[1]
            local_file = args[2]
            ...
            resp = requests.post(NOTESERVER_URL + '/' + remote_dir + '?protected=' + ('true' if protected else 'false'), descr, headers=auth_headers)
            print(resp.status_code)
            ...
            remote_note = args[1]
            resp = requests.delete(NOTESERVER_URL + '/' + remote_note, headers=auth_headers)
            print(resp.status_code)
